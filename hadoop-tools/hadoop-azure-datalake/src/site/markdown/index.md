...
* User and group information returned by API `listStatus` and `getFileStatus`
  is in the form of the GUID associated in Azure Active Directory.
...
The following sections describe the OAuth2 configuration in `core-site.xml`.
...
### <a name="Client_Credential_Token" />Using Client Credential
...
**Do not share refresh url, client id, and credential. They must be kept secret.**
...
The property `fs.adl.oauth2.access.token.provider.type` can be omitted
because its default value is `ClientCredential`.
...
commands demonstrate access to a storage account named `yourcontainer`.
...
In many Hadoop clusters, the `core-site.xml` file is world-readable. To
protect these credentials, it is recommended that you use the credential
provider framework to securely store them and access them.
...
% hadoop credential create fs.adl.oauth2.credential -value 456 \
    -provider localjceks://file/home/foo/adls.jceks
% hadoop credential create fs.adl.oauth2.refresh.url -value 789 \
    -provider localjceks://file/home/foo/adls.jceks
```

All credential properties except `fs.adl.oauth2.access.token.provider.type`
can be stored in a credential store.

The above commands create a local credential store. For jobs running on many
nodes, you either have to distribute the store file to the same path on those
nodes, or use the following provider path for HDFS:

```
jceks://hdfs/home/foo/adls.jceks
```
...
Specify property `fs.adl.oauth2.access.token.provider.type` if not using
Client Credential.

#### Running DistCp without Configuring core-site.xml

You may optionally add the provider path property to the `distcp` command
line instead of adding job specific configuration to `core-site.xml`.
